...
let process_row_validate row_num = function
  ...
  | xs ->
    ...
    let* oprichtings_datum = do_col_optional' mk_oprichtingsdatum_r "oprichtings_datum" in
    ...
    let* directeur_algemeen_geslacht = do_col_optional' mk_directeur_algemeen_geslacht "directeur_algemeen_geslacht" in
    let* directeur_algemeen_voorletters = do_col_optional' mk_directeur_algemeen_voorletters "directeur_algemeen_voorletters" in
    ...
    let* bestuursvoorzitter_tussenvoegsel = do_col_optional' mk_bestuursvoorzitter_tussenvoegsel "bestuursvoorzitter_tussenvoegsel" in
    let* bestuursvoorzitter_achternaam = do_col_optional' mk_bestuursvoorzitter_achternaam "bestuursvoorzitter_achternaam" in
    ...
    let bestuurgpenningmeester = mk_bestuurspenningmeester_option row_num !col_num
    ...
    let* bestuurslid3_geslacht = do_col_optional' mk_bestuurslid_geslacht "bestuurslid3_geslacht" in
    let* bestuurslid3_voorletters = do_col_optional' col_bestuurslid_voorletters "bestuurslid3_voorletters" in
    let* bestuurslid3_tussenvoegsel = do_col_optional' mk_bestuurslid_tussenvoegsel_r "bestuurslid3_tussenvoegsel" in
    let* bestuurslid3_achternaam = do_col_optional' mk_bestuurslid_achternaam "bestuurslid3_achternaam" in
    ...
    let* bestuurslid4_geslacht = do_col_optional' mk_bestuurslid_geslacht "bestuurslid4_geslacht" in
    let* bestuurslid4_voorletters = do_col_optional' col_bestuurslid_voorletters "bestuurslid4_voorletters" in
    let* bestuurslid4_tussenvoegsel = do_col_optional' mk_bestuurslid_tussenvoegsel_r "bestuurslid4_tussenvoegsel" in
    let* bestuurslid4_achternaam = do_col_optional' mk_bestuurslid_achternaam "bestuurslid4_achternaam" in
    ...
    let* bestuurslid5_geslacht = do_col_optional' mk_bestuurslid_geslacht "bestuurslid5_geslacht" in
    let* bestuurslid5_voorletters = do_col_optional' col_bestuurslid_voorletters "bestuurslid5_voorletters" in
    let* bestuurslid5_tussenvoegsel = do_col_optional' mk_bestuurslid_tussenvoegsel_r "bestuurslid5_tussenvoegsel" in
    let* bestuurslid5_achternaam = do_col_optional' mk_bestuurslid_achternaam "bestuurslid5_achternaam" in
    ...
    let* bestuurslid6_geslacht = do_col_optional' mk_bestuurslid_geslacht "bestuurslid6_geslacht" in
    let* bestuurslid6_voorletters = do_col_optional' col_bestuurslid_voorletters "bestuurslid6_voorletters" in
    let* bestuurslid6_tussenvoegsel = do_col_optional' mk_bestuurslid_tussenvoegsel_r "bestuurslid6_tussenvoegsel" in
    let* bestuurslid6_achternaam = do_col_optional' mk_bestuurslid_achternaam "bestuurslid6_achternaam" in
    ...
    let* cpfinaanvragen_voorletters = do_col_optional' mk_cpiaanvragen_voorletters "cpfinaanvragen_voorletters" in
    let* cpfinaanvragen_tussenvoegsel = do_col_optional' mk_cpiaanvragen_tussenvoegsel "cpfinaanvragen_tussenvoegsel" in
    let* cpfinaanvragen_achternaam = do_col_optional' mk_cpiaanvragen_achternaam "cpfinaanvragen_achternaam" in
